<template>
    <li class="lin_left_x" v-bind:style="{display: activeBlock}">
        ...
        <template v-for="(item, index) in route.steps">
            <template v-if="item.type == 5">
                <ul class="line_long" data-step-index="0">
                    <li class="line_long03"><p class="round"
                        ...
                        <p><span>步行至 <b
                                class="path_end">{{ (null == route.steps[index + 1] ? "目的地" : route.steps[index + 1].vehicle.start_name)
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.route.steps.forEach((step, idx) => {
